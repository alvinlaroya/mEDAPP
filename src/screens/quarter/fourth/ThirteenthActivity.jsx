...
function FirstActivity() {
  ...
  useEffect(() => {
    if (checkKey) {
      const thingToSay = "Add quantities up to 10";
      Speech.speak(thingToSay);
    }
  }, [checkKey]);

  const submitKeyHandler = () => {
    if (actKey === "medappq4act13") {
      setCheckKey(true);
    } else {
  ...
  return checkKey ? (
    <View style={styles.container}>
      <View style={{ padding: 10 }}>
        ...
        <Text style={{ fontSize: 19 }}>Add quantities up to 10</Text>
      </View>
      <View style={{ padding: 10 }}>
      ...
      <View style={{ padding: 10 }}>
        <Button
          ...
          onPress={async () => {
            ...
            try {
              const value = await AsyncStorage.getItem("@act13");
              const total = Number(value) + 1;
              if (total > 100) return;
              await AsyncStorage.setItem("@act13", JSON.stringify(total));
            } catch (e) {
              // saving error
            }
          }}
