...
function FirstActivity() {
  ...
  useEffect(() => {
    if (checkKey) {
      const thingToSay = "Quantity of a set of objects";
      Speech.speak(thingToSay);
    }
  }, [checkKey]);

  const submitKeyHandler = () => {
    if (actKey === "medappq3act7") {
      setCheckKey(true);
    } else {
  ...
  return checkKey ? (
    <View style={styles.container}>
      <View style={{ padding: 10 }}>
        ...
        <Text style={{ fontSize: 19 }}>Quantity of a set of objects</Text>
      </View>
      <View style={{ padding: 10 }}>
      ...
      <View style={{ padding: 10 }}>
        <Button
          ...
          buttonColor="green"
          onPress={async () => {
            navigation.navigate("Activity7");
            try {
              const value = await AsyncStorage.getItem("@act7");
              const total = Number(value) + 1;
              if (total > 100) return;
              await AsyncStorage.setItem("@act7", JSON.stringify(total));
            } catch (e) {
              // saving error
            }
          }}
